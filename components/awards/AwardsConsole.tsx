...
/**
 * Awards ceremony — coordinator controls.
 *
 * Enter the final rankings once (table or paste-from-CSV), then drive the
 * projector and every phone through the staged reveal:
 *   hidden → 3rd → 2nd → 1st → full leaderboard
 */
export function AwardsConsole({ teams }: { teams: Team[] }) {
...
const STAGE_OPTIONS: { id: CeremonyStage; label: string; desc: string; tint: string }[] = [
  ...
  { id: 'leaderboard', label: '📊 Show full leaderboard', desc: 'Every team in ranked order, DQ at bottom.', tint: '#22D3EE' },
];
...
// ─── Rankings editor (one row per team + paste-from-CSV) ────────────────────────
...
function PasteCsvModal({
  ...
  const parse = () => {
    try {
      ...
      if (out.length === 0) {
        setErr('Could not parse any rows. Expected each line like:  1\\t7\\tNorthwind\\t42.00');
        return;
      }
      onParsed(out);
    } catch (e) {
    ...
    <div className="fixed inset-0 z-[400] flex items-center justify-center bg-bg/80 p-5 backdrop-blur-md">
      <div className="w-full max-w-[720px] rounded-2xl border border-line-2 bg-surface p-6 shadow-soft-lg">
        ...
        <textarea
          ...
          placeholder={`1\t7\tNorthwind\t42.00\n2\t19\tByteForge\t40.33\n3\t22\tSentinels\t38.00\n...`}
