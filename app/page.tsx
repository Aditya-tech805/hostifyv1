...
import { usePublicReveal } from '@/lib/data';
import { ParticipantAwards } from '@/components/awards/ParticipantAwards';
...
function ParticipantShell() {
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (res.status === 401) {
          ...
          window.localStorage.removeItem(STORAGE_KEYS.team);
          setTeam(null);
          return;
        }
        if (!res.ok) {
  ...
  return (
    <>
      ...
      {showLanding && (
        <>
          {reveal && reveal.stage !== 'idle' && (
            <div className="mx-auto max-w-[720px] px-5 pt-8">
              <ParticipantAwards results={reveal} own={null} />
            </div>
          )}
      ...
      {showDashboard && team && (
        <main className="mx-auto max-w-[720px] px-5 pb-20 pt-6">
          <ParticipantAwards results={reveal} own={team} />
          <div className="animate-view-in">
